...
# פונקציה מעודכנת לשאיבת תוצאות הריצה האחרונה של ה-Actor עם ניסיון חוזר
async def run_apify_actor():
    ...
    for attempt in range(max_retries):
        try:
            ...
            url = f"{APIFY_API_URL}/acts/{APIFY_ACTOR_ID}/runs?limit=2&desc=1"  # מבקש 2 ריצות כדי לבדוק גם את הקודמת
            headers = {
